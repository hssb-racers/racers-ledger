...
fn main() {
    ...
    info!("connect port: {}, listen port: {}", opts.connect_port, opts.listen_port);

    let connect_destination = format!("ws://localhost:{}/racers-ledger/", opts.connect_port);
    let (mut socket, response) = connect(Url::parse(connect_destination.as_str()).unwrap()).expect(format!("Can't connect to {}", connect_destination).as_str());
